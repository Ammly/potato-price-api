...
class TestPrices:
    def test_estimate_unauthorized(self, client):
        response = client.post(
            "/prices/estimate",
            data=json.dumps({"location": "Nairobi", "logistics_mode": "wholesale"}),
            ...
            data=json.dumps(
                {
                    ...
                    "overrides": {"Nairobi": 120.0, "Nakuru": 110.0},
                }
            ),
            content_type="application/json",
            headers={"Authorization": f"Bearer {auth_token}"},
    ...
    def test_estimate_invalid_logistics_mode(self, client, auth_token):
        response = client.post(
            "/prices/estimate",
            data=json.dumps({"location": "Nairobi", "logistics_mode": "invalid_mode"}),
            content_type="application/json",
            headers={"Authorization": f"Bearer {auth_token}"},
        )
